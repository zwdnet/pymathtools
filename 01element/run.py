...
def run(gpus, user, server):
    ...
    if gpus == "all":
        ...
        # 运行指定代码
        # print("测试，参数个数:", len(sys.argv), " 参数:", sys.argv)
        
        s = "ssh root@" + server +  " -p 2222 \"mypy --ignore-missing-imports /home/code/" + sys.argv[2] + "\""
        print("代码静态检查……\n")
        os.system(s)
        ...
        if sys.argv[2] == "up":
            ...
            print("输入错误，copy后面要跟up或down。")
    # 在服务器上安装指定的库
    elif gpus == "install":
        arg_len = len(sys.argv)
        if arg_len == 3:
            lib_name = sys.argv[2]
            print("正在服务器上安装"+lib_name)
            s = "ssh root@" + server +  " -p 2222 \"pip install " + lib_name + " -i https://pypi.tuna.tsinghua.edu.cn/simple\""
            print(s)
            os.system(s)
        else:
            print("输入有误!")
